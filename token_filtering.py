# Simple example of filtering out tokens by type

import spacy

# Specify the text in the file itself - this isn't really practical for long
# texts:

# text = ("Muchos años después, frente al pelotón de fusilamiento, el coronel Aureliano "
...
#         "mes de marzo, una familia de gitanos desarrapados plantaba su carpa cerca de la "
...
#         "imantados. Úrsula I guaran, su mujer, que contaba con aquellos animales para "
#         "ensanchar el desmedrado patrimonio doméstico, no consiguió disuadirlo. «Muy "
...
# Or we can just read from a file like this:
with open("cads.txt") as f:
    text = f.read()
...
pad = max(len(tok.text) for tok in doc)

for token in doc:
    # print(token.text.ljust(pad), token.pos_)
    if token.pos_ == "VERB":
        print(token.text.ljust(pad), token.pos_, token.lemma_)
